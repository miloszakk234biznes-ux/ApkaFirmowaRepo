/**
 * Plik: components/orders/order-attachments.tsx
 * Cel: Sekcja załączników zlecenia — upload zdjęć i dokumentów, galeria zdjęć,
 *      lista dokumentów (PDF), usuwanie. Dane przez SWR.
 * Zależności: swr, lib/fetcher, components/ui/*, next/image.
...
export function OrderAttachments({ orderId }: { orderId: string }) {
  const key = `/api/orders/${orderId}/attachments`;
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        fd.append('file', file);
        const res = await fetch(key, { method: 'POST', body: fd });
        if (!res.ok) {
  ...
  async function remove(type: 'photo' | 'document', attId: string) {
    try {
      await fetch(`${key}?type=${type}&attId=${attId}`, { method: 'DELETE' });
      mutate();
    } catch {
